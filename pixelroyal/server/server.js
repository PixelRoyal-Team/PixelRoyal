...
app.get("/captcha.png", async function (req, res, next) {
  ...
  const background = await loadImage('../src/public/images/captcha_bg.png');
...
function findCdList(userid){
  if (cdList.length == 0)
    return {}
  for (var i = 0; i < cdList.length; i++){
    if (cdList[i].userid == userid){
      return (cdList[i])
    }
  }
  return {}
}
function updateCdUser(userid, placetime, timer){
  if (cdList.length == 0)
    return false
  for (var i = 0; i < cdList.length; i++){
    if (cdList[i].userid == userid){
      cdList[i].timer = timer
      cdList[i].placetime = placetime
      return true
    }
  }
  return false
...
io.on('connection', (socket) => {
  ...
  socket.on('place', (data) => {
    const { x, y, color, token } = data;
    const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(process.env.KEY), Buffer.from(process.env.IV))
    var decryptedToken = decipher.update(token, 'hex', 'utf8') +
      decipher.final('utf8')
    if (!decryptedToken || !JSON.parse(decryptedToken).id)
      return
    else 
      decryptedToken = JSON.parse(decryptedToken)
    const user = findCdList(decryptedToken.id)
    const now = new Date().getTime()
    if (user.userid){
      var placetime = user.placetime 
      var timer = user.timer 
      var newTimer = timer-Math.floor((now-placetime)/1000) + 1
      if (newTimer < 0)
        newTimer = 1

      if (newTimer<60){
        updateCdUser(decryptedToken.id, now, newTimer)
        io.to(socket.id).emit("updateCooldown", {placetime: now, timer: newTimer})
      } else 
        return
    } else {
      cdList.push({userid: decryptedToken.id, placetime: now, timer: 1})
    }
